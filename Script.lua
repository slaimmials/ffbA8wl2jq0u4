...
gui.objs["MainFrame"] = Instance.new("Frame", gui.objs["Gui"])
...
function gui:Page(Name)
    ...
    spawn(function()
        local time = 0
        if _G.OptimizeMenu == true then time = 0.1 end
        while wait(time) do
...
function gui:Slider(Page, Name, Max, NumberInList, Dependence)
    if gui["pages"][Page] ~= nil then
        ...
        local text = Instance.new("TextLabel", obj)
        text.Size = UDim2.fromOffset(75,15)
        text.Position = UDim2.fromOffset(55,0)
        text.BackgroundTransparency = 1
        text.TextXAlignment = Enum.TextXAlignment.Left
        text.TextSize = 8
        text.Text = Name
        ...
        num.Size = UDim2.fromOffset(50,15)
...
function gui:MultiDropDown(Page, Name, Params, NumberInList)
    if gui["pages"][Page] ~= nil then
        ...
        spawn(function()
            local time = 0
            if _G.OptimizeMenu == true then time = 0.08 end
...
gui:Slider("Visuals", "Crosshair size", 20, 2)
...
if game:GetService("ReplicatedStorage"):FindFirstChild("GetPlayerData", true) then
    ...
    gui:DropDown("Misc", "Target player", {"Murderer", "Sheriff", "All"})
    gui:CheckBox("Misc", "Grab gun")
...
gui:CheckBox("HvH", "Fakelags", nil, nil, true)
gui:Slider("HvH", "Fakelags Size", 10)
gui:CheckBox("HvH", "Anti-Aim", nil, nil, true)
gui:DropDown("HvH", "Yaw", {"Down","Up","Front","Default"})
gui:DropDown("HvH", "Pitch", {"Backwards", "Sideways", "Random", "Front", "Lock"--[[, "Jitter"]], "Zero", "Spin", "Frik","Default"})
gui:Slider("HvH", "Jitter", 180, 1)
gui:CheckBox("HvH", "Random Jitter", 1)
gui:Slider("HvH", "Spin Speed", 360, 2)
gui:CheckBox("HvH", "Left sided", 2)
gui:CheckBox("HvH", "Wallshot", 2)
...
LinesSeparator.Position = UDim2.fromOffset(10,0)
LinesSeparator.BorderSizePixel = 0
local CodeFrame = Instance.new("Frame", LuaPage)
CodeFrame.BackgroundColor3 = Color3.fromRGB(68, 68, 68)
CodeFrame.Size = UDim2.fromOffset(237,162)
CodeFrame.Position = UDim2.fromOffset(12,0)
CodeFrame.BorderSizePixel = 0
CodeFrame.ClipsDescendants = true
local Lines = {}
local LineNumber = Instance.new("TextLabel")
LineNumber.BackgroundTransparency = 1
LineNumber.Position = UDim2.fromOffset(-10,0)
LineNumber.Size = UDim2.fromOffset(10,10)
LineNumber.Text = "1"
LineNumber.TextScaled = true
LineNumber.TextColor3 = Color3.fromRGB(150,150,150)
local LineCode = Instance.new("TextLabel")
LineCode.BackgroundTransparency = 1
LineCode.Position = UDim2.fromOffset(0,0)
LineCode.Size = UDim2.fromOffset(8000,10)
LineCode.Text = ""
LineCode.TextSize = 8
LineCode.TextColor3 = Color3.fromRGB(150,150,150)

--LineNumber.Parent = LinesSeparator7
local lua = {}
function lua:CreateLine()
    lua[#lua+1] = {}
    local label = LineNumber:Clone()
    label.Parent = LinesSeparator
    label.Position = UDim2.fromOffset(-10,(#lua-1)*10)
    label.Text = #lua
    local codelabel = LineCode:Clone()
    codelabel.Parent = CodeFrame
    codelabel.Position = UDim2.fromOffset(0,(#lua-1)*10)
    codelabel.TextXAlignment = Enum.TextXAlignment.Left
    codelabel.Text = ""
    lua[#lua]["NumLabel"] = label
    lua[#lua]["CodeLabel"] = codelabel
    lua[#lua]["Code"] = ""

    spawn(function()
        while wait() do
            codelabel.Text = lua[tonumber(label.Text)]["Code"]
        end
    end)
end
lua:CreateLine()
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    elseif setInvisibleC then
        setInvisibleC = false
        for _,part in pairs(lplr.Character:GetChildren()) do
            if part:IsA("BasePart") or part.ClassName == "MeshPart" and part.Name ~= "HumanoidRootPart" and part.Name:find("Fake") == nil and part.Name:find("fake") == nil then
                part.Transparency = 0
                if part.Name:find("Head") and part:FindFirstChild("face") then
...
spawn(function()
    while task.wait() do
        if gui:IsChecked("HvH", "Fakelags") then
            flTimer += 1
            ...
            if v and (gui:IsChecked("Visuals", "Team check") and v.Team and v.Team.Name ~= lplr.Team.Name) or not gui:IsChecked("Visuals", "Team check") then
                local success, fsdf = pcall(function()
                    ...
                    if onScreen == true then
                        ...
                        if gui:IsExist("Visuals", "Highlight roles", 3) then
                            if gui:IsChecked("Visuals", "Highlight roles", 3) then
                        ...
                        elseif gui:IsChecked("Visuals", "Enabled") and gui:IsExist("Visuals", "Roles", 3) then
                            if gui:IsExist("Visuals", "Roles", 3) then
                                Team.Position = Vector2.new((vector.X-downPos/4)+(downPos/2)/2,vector.Y+(downPos*2-downPos/2))
                                Team.Color = InterColor
                                pcall(function()
                                    if gui:IsChecked("Visuals", "Roles", 3) and roles[plr] ~= nil then
                                        --spawn(function()
                                        Team.Text = roles[plr] or "No role"
                                        Team.Visible = true
                                        --end)
    ...
    if gui:IsExist("Misc", "Grab gun") then
        while wait() do
            if gui:IsChecked("Misc", "Grab gun") then
                if game:GetService("Workspace"):FindFirstChild("Normal") and game:GetService("Workspace"):FindFirstChild("Normal"):FindFirstChild("GunDrop") then
                    pcall(function ()
                        wait(0.3)
...
spawn(function ()
    ...
    while wait(0.01) do
        if gui:IsExist("Misc", "Coin grabber") and gui:IsChecked("Misc", "Coin grabber") then
            local InGame = pcall(function() local coins = game:GetService("Players").LocalPlayer.PlayerGui.MainGUI.Game.CoinBags.Container.Coin.CurrencyFrame.Icon.Coins local ass = coins.Text end)
            if InGame then
                if game:GetService("Workspace"):FindFirstChild("Normal") and grabbing == false then
                    grabbing = true
                    local coins = game:GetService("Workspace"):FindFirstChild("Normal"):FindFirstChild("CoinContainer")
                    local countCoins = tonumber(game:GetService("Players").LocalPlayer.PlayerGui.MainGUI.Game.CoinBags.Container.Coin.CurrencyFrame.Icon.Coins.Text)
                    local lastPos = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame
                    local coin = nil
                    repeat
                        if coins:GetChildren(math.random(1,#coins:GetChildren())).Transparency ~= 1 then
                            coin = coins:GetChildren(math.random(1,#coins:GetChildren()))
                        end
                        wait(0.01)
                    until coin ~= nil
...
spawn(function()
    repeat
        ...
        for i,v in pairs(game:GetService("Workspace"):GetDescendants()) do
            if v.ClassName == "Humanoid" and v.Parent:FindFirstChild("Head") ~= nil and v.Parent:FindFirstChild("HumanoidRootPart") ~= nil and game.Players:GetPlayerFromCharacter(v.Parent) == nil then
	...
	if Aimsettings.Aiming or gui:IsChecked("Aimbot", "Semi-Silent") or gui:IsChecked("Aimbot", "Instant")then
	    ...
	    if closest_char ~= nil and closest_char:FindFirstChild("HumanoidRootPart") and closest_char:FindFirstChild("Humanoid") and closest_char:FindFirstChild("Humanoid").Health > 0 then
            ...
            if gui:IsChecked("Aimbot", "Wall check") then
                local ray = game:GetService("Workspace"):Raycast(game:GetService("Players").LocalPlayer.Character.Head.Position, closest_char[Aimsettings.Aimbot_AimPart].Position - game:GetService("Players").LocalPlayer.Character.Head.Position)
                if not ray.Instance:IsDescendantOf(closest_char) then
                    if ray ~= nil and gui:IsChecked("Aimbot", "Hitscan") then
                        local Target = closest_char[Aimsettings.Aimbot_AimPart]
            ...
            if work then
                local aimToPos = closest_char[Aimsettings.Aimbot_AimPart].Position
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    if gui:IsExist("Misc", "Tp players") == true then
        if gui:IsChecked("Misc", "Tp players") then
            local selected = gui:GetDropItem("Misc", "Target player")
...
for i,v in pairs(game:GetService("ReplicatedStorage"):GetDescendants()) do
    if v.Name:find("Head") or v.Name:find("Rot") then
        headRot = v;
    end
end
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    if gui:IsChecked("HvH", "Anti-Aim") then
        local YawParam = gui:GetDropItem("HvH", "Yaw")
        local Parameter = gui:GetDropItem("HvH", "Pitch")
        if Parameter == "" and YawParam == ""  then
            ...
            if headRot ~= nil and (headRot.ClassName == "RemoteEvent" or headRot.ClassName == "RemoteFunction") then
                if YawParam == "Down" then
                    headRot:FireServer(CFrame.new(Vector3.new(0,0,0))*CFrame.Angles(0,0,math.rad(-90)))
                elseif YawParam == "Front" then
                    headRot:FireServer(CFrame.new(Vector3.new(0,0,0))*CFrame.Angles(0,0,math.rad(0)))
                elseif YawParam == "Up" then
